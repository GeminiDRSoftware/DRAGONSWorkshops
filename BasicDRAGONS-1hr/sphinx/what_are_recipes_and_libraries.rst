...
The tool |showrecipes| can be used to see which recipe will be selected by
DRAGONS and inspect other recipes matching the input data.  The tools
basic usage signature is::

    showrecipes fitsfile_name
...
quality of the data.  The mode can be specified in |showrecipes| with the
``-m`` flag.
